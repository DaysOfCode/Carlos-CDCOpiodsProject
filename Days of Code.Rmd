...
cdcMonthlyStateData        <- read_tsv("Data/statefull.txt")
...
cdcAnnualData        <- read_tsv("Data/statefull_annual.txt")
...
cdcAnnualData <- cdcAnnualData %>% select(State,Year,Deaths,Population,Crude.Rate)

# Calculate the US totals
us_agg_data <- cdcAnnualData %>% group_by(Year)  %>% summarise(Deaths=sum(Deaths), Population = sum(Population),Crude.Rate = (sum(Deaths) / sum(Population)) * 100000)
